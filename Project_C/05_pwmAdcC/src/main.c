...
int main() {
    ...
    while (1) {
        ...
        printf("*** ADC channel 0: %d -", adcSample_1);
        printf("- ADC channel 1: %d ***", adcSample_2);
        ...
        if (adcSample_2 < ADC_MIN_READVALUE)
            adcSample_2 = 0; 
        project_pwm_set_chan_level((uint)adcSample_1, PWM_GPIO_CH0A); //LEFT Motor and LED
        project_pwm_set_chan_level((uint)adcSample_2, PWM_GPIO_CH1A);//RIGHT Motor and LED
        printf("***\n"); //Para que en una linea se vea de la forma...
        // ADC channel 0: ### -- ADC channel 1: ### PWM A: ## PWM B: ## 
